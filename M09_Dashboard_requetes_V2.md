...
- un dashboard réel `[TRAINING] M09 - <participant>` ;
...
```text
[TRAINING] M09 - <identifiant-participant>
```
...
5. Remplacez la valeur entre chevrons par votre identifiant autorisé.
...
- Logs : scénario `checkout-investigation` du service `training-<participant>-svc`
- APM : `eu-interfaces`, opération `console`
- RUM : application `peopulse`, agrégats uniquement
- Monitors : monitor `[TRAINING] M08 Log errors - <participant>`
...
```text
service:training-<participant>-svc source:powershell-training @training_scenario:checkout-investigation
```
...
- le nom commence par `[TRAINING] M09` et contient votre identifiant ;
